package org.hy.common.callflow.junit.cflow036Shell;
...
import org.hy.common.callflow.junit.cflow036Shell.program.Program;
...
public class JU_CFlow036 extends AppInitConfig
{
    ...
    {
        new JU_CFlow036().test_CFlow036();
    }
    
    
    
    public JU_CFlow036() throws Exception
    {
        if ( !$isInit )
        {
            $isInit = true;
    ...
    @Test
    public void test_CFlow036() throws ClassNotFoundException, IOException
    {
        test_CFlow036_Inner();
        System.out.println("\n");
        test_CFlow036_Inner();  // 第二次运行时间，明显变快
    }
    
    
    
    private void test_CFlow036_Inner() throws ClassNotFoundException, IOException
    {
        // 初始化被编排的执行对象方法
        ...
        Return<Object> v_CheckRet = CallFlow.getHelpCheck().check(v_Shell);
        if ( !v_CheckRet.get() )
        {
        ...
        // 导出
        System.out.println(CallFlow.getHelpExport().export(v_Shell));
    }
