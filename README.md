...
------
    ...
    2. 18种高级元素。
        ...
        2.3.  JAVA爪哇元素，在Java中动态的嵌入Java代码，编译Java源代码动态构建运行时的类。
        
        2.4.  PY蟒蛇元素，在Java中嵌入Python代码。
        
        2.5.  GV酷语元素，在Java中嵌入Groovy代码。也可嵌入Java代码。
        
        2.6.  SH脚本元素，在Java中嵌入Shell代码。可远程上传及下载文件。
        
        2.7.  CMD命令元素，衍生于执行元素，执行操作系统命令。
        
        2.8.  FTP文传元素，上传下载文件和目录。如果是目录将上传或下载它中所有文件。
        
        2.9.  MIO存对元素，Minio上传下载文件和目录、分享文件。如果是目录将上传或下载它中所有文件。
        
        2.10. SQL库元素，衍生于执行元素，支持关系数据库CRUD、DDL、DML、XSQL组等操作。同时支持IoTDB时序数据库的CRUD、DDL、DML操作。
        
        2.11. CQL图谱元素，衍生于执行元素，图数据库的查询、插入、更新、删除、DDL、DML等操作。
        
        2.12. MDB向量元素，衍生于执行元素，向量库Milvus的DML（标量&向量查询、插入、更新、删除）、DDL（创建集合、删除、存在、加载、释放）。
        
        2.13. API接口元素，衍生于执行元素，用于API接口请求访问。
        
        2.14. ZIP压缩元素，衍生于执行元素，文件、文件流、多个文件或目录压缩成一个压缩包。
        
        2.15. DEC解压元素，衍生于执行元素，压缩包解压。
        
        2.16. ENF密文元素，衍生于执行元素，对文件或文件流的加密存储。密钥可由MD5自动生成。
        
        2.17. DEF解文元素，衍生于执行元素，对加密文件的解密。
        
        2.18. MR归纳元素，衍生于执行元素，将多个小集合或对象归纳成一个大集合。
        
    3. 10种拓展元素。
    
        3.1.  IOTG读元素，衍生于执行元素，用于读取PLC数据。依赖于PLC微服务。
        
        3.2.  IOTS写元素，衍生于执行元素，用于向PLC写入数据。依赖于PLC微服务。
    ...
    4. 2种隐性路由。
        ...
        4.1 路由元素，衔接上下两个或多个元素，表示编排可能的走向。

        4.2 模拟元素，在虚拟环境中模拟生产环境下编排数据数据流，可模拟正常数据流及异常数据流，被模拟的元素不会真的执行。
        
    5. 4种编排路由。
        
        5.1 成功路由，元素执行成功时走的路。
        
        5.2 异常路由，元素执行出时异常时走的路。
        
        5.3 真值路由，条件逻辑元素、计算元素（逻辑类型）与等待元素（计数器类型），结果为真时走的路。
        
        5.4 假值路由，条件逻辑元素、计算元素（逻辑类型）与等待元素（计数器类型），结果为假时走的路。
        
    6. 3种编排循环。
    
        6.1 除了普通路由（成功、异常、真值、假值路由）外，还有3种循环路由（While循环、For循环、自循环）。
        
        6.2 While循环路由，下层元素回流到上层元素时走的路。
        
        6.3 For循环路由，与循环元素配合（至少有一个），回流到循环元素的路。
        
        6.4 自循环路由，某个元素的下一下路由节点指向自己，可用于实现递归操作。
        
        6.5 路由与循环可以组合后共存（16种），即：成功路由+For循环路由组成一条成功时For循环的路由。
        
    7. 常用的系统预设变量。
    
        7.1. :CallFlowWorkID，编排实例ID的变量名称。
        
        7.2. :CallFlowContext，编排实例上下文的变量名称。它作为参数向Java方法传参，可读可写其内容。
        
        7.3. :CallFlowErrorResult，编排实例异常的变量名称。仅最后一次异常的信息。

        7.4. :CallFlowBeginTime，编排执行开始时间的变量名称。

        7.5. :CallFlowTimeLen，编排执行累计时长（单位：毫秒）的变量名称。它是动态值，每次得到的值均不一样。
...
图中右侧小红点的表示：此编排元素无Mock模拟。否则表示，有Mock模拟。

同时图中小红点也表示：不能修改编排续跑类型，必须是主动重做模式。
...
|分类|说明|精准度|标记成本|
|:--------:|:--------|:--------:|:--------:|
|异常标记|编排执行异常时自动标记路由点|精准|自动|
|元素标记|编排元素上预先预设续跑类型|循环、多分支等特殊情况无法精准定位|容易|
|结果标记|编排结果上定制化标记续跑类型|精准|较难|
